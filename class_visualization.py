...
class ClassVisualization:
    ...
    def create_class_visualization(self, target_y, class_names, model, **kwargs):
        ...
        ## Let sy(I) be the score that a convolutional network assigns to 
        # the image I for class y;
        ...
        def compute_gradient(model, img_var):
            ...
            print("img",img_var.size()) # [1, 3, 224, 224]
            print(scores.size()) # [1, 1000]
            
            score = scores[0,target_y] 
            print("y,",score.size() )
            RI = l2_reg * (torch.linalg.norm(img_var))**2
            score -= RI
            
            score.backward()
            
            step = learning_rate * img_var.grad
            step /= torch.linalg.norm(img_var.grad)
            img_var.data += step
