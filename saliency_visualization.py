...
class SaliencyMap:
    def compute_saliency_maps(self, X, y, model):
        ...
        scores = model(X_var) # (N, C)
        print(scores.size())
            
        score = scores[range(N), y].sum() # 
        print(score.size())
        score.backward() # 

        abs_X_grad =torch.abs( X_var.grad) # [5, 3, 224, 224]
        abs_X_grad_max = torch.max(abs_X_grad, axis=1) # [5, 224, 224]
        # print(abs_X_grad.shape)
        # print(abs_X_grad_max[1])
        saliency = abs_X_grad_max[0]
        # print(len(saliency))
